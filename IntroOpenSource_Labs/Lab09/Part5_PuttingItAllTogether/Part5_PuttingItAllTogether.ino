...
{  
  ...
  {
    temp = message;
    lcd.print(String(message[1]) + String(message[2]) + ":" + String(message[3]) + String(message[4]));
  } 
}

unsigned long StartTimer(String start)
{
  if (start == "s")
  {
...
unsigned long StopTimer(String stop)
{
  if (stop == "p")
  {
    unsigned long time = millis();    
...
void loop() {
  ...
  unsigned long startTime = StartTimer(message);
  unsigned long stopTime = StopTimer(message);
  ...
  if (startTime != 0)
  {
    while(stopTime == 0)
    {
      ...
      {
        lcd.setCursor(5, 0);
        lcd.print("::"+String(0) + String(currentTime));
      }  
      else
      {
        lcd.setCursor(5, 0);
      ...
      if (currentTime >60)
      {
        currentTime = 0;
        lcd.setCursor(4, 0);
        lcd.print(String(temp[4]).toInt() + 1);
      }
